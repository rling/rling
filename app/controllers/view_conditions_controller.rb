class ViewConditionsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @view_condition.save
        format.html { redirect_to([@page,@view_condition], :notice => 'View condition was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @view_condition.update_attributes(params[:view_condition])
        format.html { redirect_to([@page,@view_condition], :notice => 'View condition was successfully updated.') }
        format.xml  { head :ok }
      else
