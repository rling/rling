...
When /^I press "([^"]*)" for "([^"]*)" on the model_submissions index page$/ do |arg1, arg2|
  if arg1 == "Show"
    ...
    visit category_remove_object_model_model_submission_path(@object_model,@categorization)
    puts 333333333333333
    puts page.find("#middle").native.inner_html
  elsif arg1 == "Comments"
...
Given /^I have a model_submission with comment in model_submissions index page$/ do
  ...
  @comment_submission = Factory(:comment_submission, :model_submission_id=>@model_submission.id)
  @comment_data = Factory(:comment_data, :comment_component_id=>@comment_component.id,:comment_submission_id=>@comment_submission.id, :data_value=>"This is very good work by Heurion Consulting.")
end
