...
describe FormSubmission do
 ...
 before (:each) do
  ...
  it "belongs to a object form" do
   ...
   @form_data1=FormData.create!( @form_data_attributes.merge(:form_submission_id=>@form_submission.id))
   @form_data2=FormData.create!( @form_data_attributes.merge(:form_submission_id=>@form_submission.id))
   @form_submission.should have(2).form_datas
  end


  it "testing the association ofpage variable" do
  FormSubmission.reflect_on_association(:form_datas).should_not  be_nil
  end
  ...
  #*******************Tests instance methods ***************************#
    it "should test emailable format method " do
     ...
     @form_data1=FormData.create!( @form_data_attributes.merge(:form_submission_id=> @form_submission.id, :form_component_id => @form_component1.id ,:data_value=>'value'))
     ...
     @form_data2=FormData.create!( @form_data_attributes.merge(:form_submission_id=> @form_submission1.id, :form_component_id => @form_component2.id ,:data_value=>'value'))
     ...
     @form_data3=FormData.create!( @form_data_attributes.merge(:form_submission_id=> @form_submission2.id, :form_component_id => @form_component3.id ,:data_value=>'1'))
